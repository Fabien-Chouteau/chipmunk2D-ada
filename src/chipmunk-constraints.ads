...
is
   ...
   type cpConstraintPreSolveFunc is
     access procedure  (constraint : cpConstraint; space : cpSpace)

   with Convention => C;

   type cpConstraintPostSolveFunc is
     access procedure  (constraint : cpConstraint; space : cpSpace)
   ...
   procedure SetCollideBodies
     (constraint : cpConstraint; collideBodies : cpBool);
   --
   pragma Import (C, SetCollideBodies, "cpConstraintSetCollideBodies");

   function GetPreSolveFunc
     (constraint : cpConstraint) return cpConstraintPreSolveFunc;
   --
   pragma Import (C, GetPreSolveFunc, "cpConstraintGetPreSolveFunc");

   procedure SetPreSolveFunc
     (constraint : cpConstraint; preSolveFunc : cpConstraintPreSolveFunc);
   --
   pragma Import (C, SetPreSolveFunc, "cpConstraintSetPreSolveFunc");

   function GetPostSolveFunc
     (constraint : cpConstraint) return cpConstraintPostSolveFunc;
   --
   pragma Import (C, GetPostSolveFunc, "cpConstraintGetPostSolveFunc");

   procedure SetPostSolveFunc
     (constraint : cpConstraint; postSolveFunc : cpConstraintPostSolveFunc);
   --
